...
# --- Argument Parser ---
def parse_arguments():
    parser = argparse.ArgumentParser(description='Triple Pattern Backtest')
...
print(f"=== TRIPLE PATTERN BACKTEST ===")
...
def calculate_backtest_results(df, exit_periods, initial_balance=INITIAL_BALANCE, position_size_pct=POSITION_SIZE_PCT):
    """
    Backtest với chỉ 1 position tại 1 thời điểm:
    - Quản lý 1 position duy nhất
    - Sử dụng percentage của vốn
    - Close position theo bars held hoặc take profit
    """
    results = []
    balance = initial_balance
    position = None  # Chỉ 1 position
...
def export_triple_pattern_comparison_report(results_9, results_26, final_balance_9, final_balance_26, filename=None):
    if filename is None:
        ...
        filename = f"triple_pattern_{SYMBOL.lower()}_{TIMEFRAME}_{start_date_formatted}_{end_date_formatted}.md"
    
    with open(filename, 'w', encoding='utf-8') as f:
        f.write(f"# Triple Pattern Strategy Comparison - {SYMBOL} {TIMEFRAME}\n\n")
        ...
        if results_9:
            ...
            f.write(f"|------------|-----------|------|-------------|------------|-----|-------|-------------|-------------|-----------|\n")
            for r in results_9:
                f.write(f"| {r['entry_time'].strftime('%Y-%m-%d %H:%M')} | {r['exit_time'].strftime('%Y-%m-%d %H:%M')} | {r['type']} | ${r['entry_price']:.4f} | ${r['exit_price']:.4f} | ${r['pnl']:.2f} | {r['pnl_percent']:.2f}% | {r['pattern_type']} | {r['exit_reason']} | {r['bars_held']} |\n")
            f.write(f"\n")
        ...
        f.write(f"---\n")
        f.write(f"*Báo cáo được tạo tự động bởi Triple Pattern Backtest System*\n")
